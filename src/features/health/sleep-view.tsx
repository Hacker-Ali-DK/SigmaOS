...
export default function SleepView({ onBack }: SleepViewProps) {
  ...
  return (
    <div className="flex flex-col gap-6 px-4 pt-6 pb-24">
      ...
      <div className="card-hero flex flex-col items-center justify-center text-center py-8">
        <div className="flex items-center justify-center gap-8 w-full max-w-xs">
          ...
          <div className="flex flex-col items-center">
            <div className="w-20 h-20 rounded-full bg-slate-900/60 border border-indigo-500/20 flex flex-col items-center justify-center">
              ...
              <span className="text-micro text-slate-500 font-bold uppercase tracking-wider">24h Total</span>
            </div>
            <span className="text-micro text-slate-400 font-semibold mt-2">Sleep Duration</span>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-micro text-slate-400 font-semibold mt-1">Sleep Score</span>
          </div>
        </div>

        {sleepScoreDetail.status !== 'insufficient' && (
          <div className="mt-4 flex flex-col items-center">
            <span className="text-micro text-[#4CC9F0] font-bold uppercase tracking-wider">
              Based on {sleepScoreDetail.trackedCount} of {sleepScoreDetail.totalCount} factors
            </span>
            {sleepScoreDetail.recommendation && (
              <p className="text-xs text-slate-400 mt-2 max-w-sm px-4 leading-relaxed italic">
                "{sleepScoreDetail.recommendation}"
              </p>
            )}
          </div>
      ...
      {/* Forms Section */}
      {showNightForm && (
        <form onSubmit={handleSaveNightSleep} className="card-tertiary flex flex-col gap-4 animate-in slide-in-from-top duration-300">
          <div className="flex items-center justify-between">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-label text-slate-400 block mb-1">Bedtime</label>
              <input
                type="time"
                value={nightBedtime}
            ...
            <div>
              <label className="text-label text-slate-400 block mb-1">Wake-up Time</label>
              <input
                type="time"
                value={nightWakeup}
          ...
          <div>
            <label className="text-label text-slate-400 block mb-1.5">Sleep Quality</label>
            ...
            <div className="flex justify-between px-1 mt-1 text-micro text-slate-500 font-bold uppercase">
              <span>Very Poor</span>
              <span>Excellent</span>
            </div>
          </div>
          ...
          <div>
            <label className="text-label text-slate-400 block mb-1">Awakenings (Optional)</label>
            <input
              type="number"
              min="0"
          ...
          <div>
            <label className="text-label text-slate-400 block mb-1">Notes (Optional)</label>
            <input
              type="text"
              placeholder="Add sleep details or dream records"
              value={nightNotes}
              onChange={(e) => setNightNotes(e.target.value)}
              className="w-full px-3 py-3 bg-slate-950 border border-slate-900 focus:border-[#3A86FF] focus:outline-none rounded-xl text-xs text-slate-200"
            />
          </div>
          ...
          {isUnusuallyLong && (
            <div className="p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/20 flex flex-col gap-2">
              ...
              <label className="flex items-center gap-2 mt-1 cursor-pointer">
                ...
                />
                <span className="text-micro text-slate-300 font-bold select-none uppercase tracking-wide">
      ...
      {showNapForm && (
        <form onSubmit={handleSaveNap} className="card-tertiary flex flex-col gap-4 animate-in slide-in-from-top duration-300">
          <div className="flex items-center justify-between">
            <h3 className="text-xs font-bold text-slate-200 uppercase tracking-wider flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-label text-slate-400 block mb-1">Start Time</label>
              <input
                type="time"
                value={napStart}
                onChange={(e) => setNapStart(e.target.value)}
                className="w-full px-3 py-3 bg-slate-950 border border-slate-900 focus:border-[#02C39A] focus:outline-none rounded-xl text-xs text-slate-200 font-bold"
              />
            </div>
            <div>
              <label className="text-label text-slate-400 block mb-1">End Time</label>
              <input
                type="time"
                value={napEnd}
          ...
          <div>
            <label className="text-label text-slate-400 block mb-1.5">Nap Quality</label>
            <div className="flex gap-1.5 bg-slate-950 p-1 rounded-xl">
              {['1', '2', '3', '4', '5'].map((r) => (
                <button
                  key={r}
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-3 bg-slate-950 border border-slate-900 focus:border-[#02C39A] focus:outline-none rounded-xl text-xs text-slate-200"
            />
          </div>
      ...
      <div className="card-primary flex flex-col gap-4">
        ...
        {sleepLog ? (
          <div className="flex flex-col gap-3">
            <div className="grid grid-cols-2 gap-4">
              <div className="card-secondary p-3 flex flex-col">
                <span className="text-micro text-slate-500 font-bold uppercase">Bedtime</span>
              ...
              <div className="card-secondary p-3 flex flex-col">
                <span className="text-micro text-slate-500 font-bold uppercase">Wake-up Time</span>
                <span className="text-sm font-extrabold text-slate-200 mt-1">
            ...
            <div className="grid grid-cols-3 gap-3 text-center">
              <div className="card-secondary p-3 flex flex-col">
                <span className="text-micro text-slate-500 font-bold uppercase">Duration</span>
              ...
              <div className="p-2.5 bg-slate-950/40 rounded-xl border border-slate-900 flex flex-col">
                <span className="text-micro text-slate-500 font-bold uppercase">Quality</span>
              ...
              <div className="p-2.5 bg-slate-950/40 rounded-xl border border-slate-900 flex flex-col">
                <span className="text-micro text-slate-500 font-bold uppercase">Awakenings</span>
      ...
      <div className="card-primary flex flex-col gap-4">
        ...
        {naps.length > 0 ? (
          <div className="flex flex-col gap-2">
            {naps.map((n) => (
              <div key={n.id} className="card-secondary p-3 flex items-center justify-between">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-extrabold text-slate-200">
                      {n.startTime} - {n.endTime}
                    </span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-bold uppercase">
                      {n.durationMinutes} min
                    </span>
                  </div>
                  {n.notes && <span className="text-micro text-slate-500 mt-1 italic">"{n.notes}"</span>}
                </div>
                <div className="flex gap-2">
                  <button 
          ...
          <span className="text-xs text-slate-500 font-medium text-center py-2">No naps recorded today.</span>
        )}
      </div>
      ...
      <div className="card-primary flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-2">
          {recentLogs.map((log) => {
            ...
            return (
              <div key={log.date} className="card-tertiary p-3 flex items-center justify-between">
                <div className="flex flex-col">
                  <div className="flex items-center gap-2">
                    <span className="text-xs font-extrabold text-slate-200">{log.date}</span>
                    {hasLegacyPhases && (
                      <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/20 font-bold uppercase">
                        Estimated / Historical
                      </span>
                    )}
                  </div>
                  <span className="text-micro text-slate-500 font-semibold mt-1">
